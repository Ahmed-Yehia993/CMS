...
public class ConsumeCustomerService {
    ...
    public String setAccountStatus(String accountNewStatus, String accountPoid){
        String xmlResponse = "";
        String xmlRequest = "	<PCM_OP_CUST_SET_STATUS_inputFlist>\r\n"
                + "					<POID>"+ accountPoid +"</POID>\r\n"
                + "					<PROGRAM_NAME>CMS SET STATUS : "+ accountNewStatus +"</PROGRAM_NAME>\r\n"
                + "					<STATUSES elem=\"0\">\r\n"
                + "	    				<STATUS>"+ accountNewStatus +"</STATUS>\r\n"
                + "		    			<STATUS_FLAGS>8</STATUS_FLAGS>\r\n"
                + "					</STATUSES>\r\n"
                + "             </PCM_OP_CUST_SET_STATUS_inputFlist>\r\n"
                ;
        try {
            System.out.println("setAccountStatus" + xmlRequest);
            xmlResponse = serviceLocator.getBRMCustServices().pcmOpCustSetStatus(0, xmlRequest);
            System.out.println("setAccountStatus" + xmlResponse);
        } catch (RemoteException | ServiceException e) {
            e.printStackTrace();
        }
        return xmlResponse;
